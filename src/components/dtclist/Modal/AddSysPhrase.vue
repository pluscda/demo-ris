    ...
    <b-container fluid>
      <main class="main-grid-dtc">
        <b-input-group prepend="片語類型" class="mb-2">
          <b-select
            :options="allowPhraseTypes"
            v-model="allow"
            v-if="allowPhraseTypes.length > 1"
          ></b-select>
          <b-input
            v-if="allowPhraseTypes.length == 1"
            readonly
        ...
        <b-input-group prepend="片語代號" class="mb-2 w-100">
          ...
          ></b-input>
          <div
            v-if="allowPhraseTypes.length > 1 && allow == '公用片語'"
            class="code-dtc"
          >
    ...
    <template v-slot:modal-footer>
      <div class="w-100">
        <b-button
          ...
          @click="$bvModal.hide('dtcAddSysPhrase')"
          >取消</b-button
        >
        <b-button
          variant="success"
...
export default {
  ...
  methods: {
    ...
    async addPhrase() {
      ...
      try {
        await axios.post("phrase/AddSystemPhrase", obj);
        ...
        this.$bvModal.hide("dtcAddSysPhrase");
      } catch (e) {
