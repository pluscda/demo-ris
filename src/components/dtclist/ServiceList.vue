<template>
  <section class="dtc-main-section">
    <header class="ask-header">收件檢核作業</header>
    <div class="dtc-search">
      <main class="left-header">
        <b-input-group prepend="身份證號">
          <b-input v-model.trim="id" :autofocus="autofocus" maxlength="50" placeholder="搜尋身份證字號"></b-input>
        </b-input-group>
        <b-input-group prepend="檢查狀態">
          <b-select :options="sizes" v-model="size" @change="searchDb"></b-select>
        </b-input-group>
        <b-input-group prepend="姓名">
          <b-input v-model.trim="name" placeholder="搜尋姓名" maxlength="50"></b-input>
        </b-input-group>
      ...
      <div></div>
      <main class="right-header" hidden>
        <b-button class="mb-2" variant="light" @click="searchDb" size="sm">匯出PDF</b-button>
        <b-button variant="info" @click="clearSearch" size="sm">匯出Excel</b-button>
      </main>
      <div></div>
    </div>
    ...
    <header class="dtc-grid-header my-dark">
      ...
      <div v-for="(item, i) in headers" :key="i" @click="sort(item)">
        {{ item.name }}
        <span v-show="item.sortDesc === null && item.name != '未完成項次'">
          <i class="fas fa-sort"></i>
        </span>
        <span v-show="item.sortDesc === false && item.name != '未完成項次'">
          <i class="fas fa-sort-up"></i>
        </span>
        <span v-show="item.sortDesc && item.name != '未完成項次'">
    ...
    >
      <div>
        <b-button variant="success" size="sm" @click="edit(item)">檢核</b-button>
      </div>
      ...
      <div
        class="dtc-ellipsis"
        style="color:red;"
        :title="item.NotExamIDs.toString()"
        v-b-tooltip.hover
      >{{ item.NotExamIDs.toString() }}</div>
    ...
    <div v-if="showFinalResult" class="final-result">
      <FinalResult fromService="true"></FinalResult>
    </div>
  </section>
</template>
...
const headers = [
  ...
  { name: "場次", key: "BatchId", sortDesc: null },
  ...
  { name: "未完成項次", key: "NotExamIDs", sortDesc: null },
...
.dtc-grid-header {
  display: grid;
  grid-template-columns: 60px repeat(7, 120px) 1fr 200px;
