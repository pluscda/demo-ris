<template>
  <section class="dtc-main-section">
    <header class="ask-header">役男清冊查詢</header>
    <div class="dtc-search">
      <main class="left-header">
        ...
        <b-input-group prepend="體檢場次" hidden>
...
const headers = [
  ...
  { name: "列管編號", key: "Unknown", sortDesc: null },
...
export default {
  ...
  methods: {
    ...
    async clearSearch() {
      this.id = "";
      this.name = this.where = this.area = this.size = this.time = null;
      this.getData();
    },
    async getData() {
      //odata3 qs
