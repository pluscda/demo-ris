<template>
  <section class="dtc-main-section">
    <header class="ask-header">報到作業</header>
    ...
    >
      <div>
        <b-button variant="success" size="sm" @click="edit(item)">報到</b-button>
      </div>
      <div>{{ item.Name }}</div>
      <div>{{ item.IdNo }}</div>
...
.dtc-grid-header {
  display: grid;
  grid-template-columns: 60px repeat(6, 120px) 1fr 1fr 120px;
