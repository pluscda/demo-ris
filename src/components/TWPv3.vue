...
<template>
	<div id="twpv2">
			...
			<b-col xl="12">
				...
				>
					...
					<template v-slot:cell(CheckBoxColumnType1)="row">
						<b-form-checkbox v-if="IfShowCheckBox(row.item)" size="lg" v-model="row.item.TWPCheckBox1Value" :value="true" :unchecked-value="false" @change="OnColumnCheckBox1Change(row.item)">{{ Text4CBColCheckBox1 }}</b-form-checkbox>
					</template>
					<template v-slot:head(CheckBoxColumnType1)="head">
						<b-form-checkbox size="lg" v-model="allChecked" :value="true" :unchecked-value="false" @change="OnAllCheckBoxChange()">{{head.label}}</b-form-checkbox>
					</template>
				</b-table>
			</b-col>
		</b-row>
		<b-row no-gutters class="mb-1" v-if="PagingEnabled">
			...
			<b-col sm="4">
				<b-pagination v-model="PagingCurrentPage" @change="OnPageChange" :total-rows="RowCount" :per-page="PagingRowPerPage" aria-controls="dataTable1" align="center">
