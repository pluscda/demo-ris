      ...
      <b-card-text class="text-left py-1 m-0">
        <b-container fluid class="px-0">
          ...
          <b-row no-gutters class="mt-1 mx-0">
            <!-- remove report-tab -->
            <b-col v-if="!showcst" sm="9" class="dtc-vue-tab title_center" style="border:1px solid #ccc;border-radius:5px ;overflow:auto">
              <!-- 這報告不要跑 -->
              <vue-tabs active-tab-color="#e74c3c" active-text-color="white" class="mx-1" style="height:calc(100vh - 300px)">
                <v-tab v-for="(item, idx) in cstTab" :key="idx" :title="item.CSTName">
            ...
            <b-col sm="3" class="px-1">
              <b-card no-body style="height:100%">
                ...
                <b-card-text class="px-1 pb-1" style="width: 100%; height: 100%;">
                  ...
                  <b-textarea
                    ...
                    placeholder="科內審查意見"
                  ></b-textarea>
                  <b-textarea v-else readonly style="height:calc(100vh - 470px)" no-resize v-model="evaluation.DxRAEComment" placeholder="科內審查意見"></b-textarea>
                  <!-- </div> -->
                </b-card-text>
              </b-card>
            </b-col>
...
export default {
  ...
  methods: {
    ...
    async saveEvaluate() {
      ...
      console.log(this.$logs("把evaluationSelected從[] => string"));
  ...
  async beforeMount() {
    ...
    this.$root.$on("dtc-copy-paste", s => {
      ...
      this.theReport.ReportContent += " " + s;
    });
    //new SmartTextarea(document.getElementById("DxRMain"));
    ...
    try {
      ...
      console.log(JSON.parse(this.theReport.StructureData1));
      ...
      //
      for (let i = 1; i < 7; i++) {
        if (this.theReport.StructureContent1 !== null) {
          //input disable
          var inputHtml = this.theReport["StructureContent" + i].replace(/<input /g, "<input disabled='disabled' ");
          //textarea disable
          var textAreaHtml = inputHtml.replace(/<textarea /g, "<textarea disabled ");
          //select disable
          var html = textAreaHtml.replace(/<select /g, "<select disabled ");
        }
        // let html = this.theReport["StructureContent" + i];
        cstdata.push({
        ...
        //  想用exg exper作
        // let valueSet = JSON.parse(this.theReport["StructureData" + i]);
        // console.log(valueSet);

        // valueSet.forEach(e => {
        //   console.log(e["name"]);
        //   console.log(e["value"]);
        //   html.replace(/e["name"]/g, "e['name']");
        // });

        console.log(this.$logs(cstdata));
      }
      console.log(this.$logs("判斷是否有癌篩報告"));
      // console.log(JSON.stringify(cstdata));
      this.theReport.CSTName1 ? (this.cstreport = false) : "";
      //資料存到v-for的[]

      // 先抓一筆資料測試
      // console.log(cstdata[0].StructureData);
      let valueSet = JSON.parse(cstdata[0].StructureData);
      ...
      if (valueSet && valueSet.length > 0) {
        ...
        valueSet.forEach(e => {
          ...
          let item = $(`#WinCST0306-1DxRSC [name="${e.name}"]`);
          console.log(item);
        });
        */
      }
      ...
      this.cstTab = cstdata;
      console.log(this.cstTab);
      console.log(this.$logs("已把資料存到v-for的[]"));
      //
      let t1 = $("#WinCST0306-1DxRSC");
      console.log(t1);
      console.log("done");
      //
      //
      ...
      //Step 2: get history list 歷史診斷報告
      //this.reportHistory = await window.axios.get(`/Report/GetVersionList?accessionNo=${this.theProcedure.AccessionNo}`)
