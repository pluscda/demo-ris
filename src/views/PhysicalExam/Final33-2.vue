  ...
  <section class="details">
    ...
    <main class="my-3-grid my-body" v-for="(item, i) in inspectsArr" :key="i">
      <div v-if="!specialCase(item)" style="color:var(--success)">
        ...
        <span v-if="getResult(item) == 3" style="color:blue;">專</span>
        <span v-if="getResult(item) == 4" style="color:rgb(253, 126, 20);">已</span>
        <span v-if="!getResult(item)" style="color:red;">未</span>
      </div>
      <div v-if="!specialCase(item)" style="color:var(--danger)">
        <img src="close.svg" width="12" height="12" v-if="getResult(item) == 2" />
        <span v-if="getResult(item) == 3" style="color:blue;">檢</span>
        <span v-if="getResult(item) == 4" style="color:rgb(253, 126, 20);">檢</span>
        <span v-if="!getResult(item)" style="color:red;">檢</span>
      </div>
      <div
      ...
      <div
        v-html="item"
        v-if="specialCase(item)"
        :style="!needBB(item) ? 'border-bottom:none;' : 'border-bottom:1px solid #bfbfbf;'"
        style="grid-column: 1/4;background: #F5F5F5; color:black;text-align:left;;padding-left:2px;"
      ></div>

      <div
        style="width:206px;border:none;"
        :style="getSealStatus(item) > 1 ? 'border-bottom:1px solid #bfbfbf !important;': ''"
      >
...
let inspectsArr = [
  ...
  "28.辨色力",
  "29.視力",
  "<span style='font-size:13px'>裸 視：右{%4} 左{%5}</span> ",
  "<span style='font-size:13px'>驗光度數：右{%6} 左{%7}</span> ",
  "<span style='font-size:13px'>矯正視力：右{%8} 左{%9}</span> ",
  "30.神經系統",
  "31.精神",
  "32.智能",
  "33.心電圖",
...
export default {
  ...
  methods: {
    ...
    getItemDetail(){ // NOTE: get item 25 and item 29 item in this function
